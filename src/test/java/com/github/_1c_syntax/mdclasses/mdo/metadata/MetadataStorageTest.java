...
class MetadataStorageTest {
  ...
  void testGetStorage() {
    ...
    assertThat(meta.values().stream().map(Metadata::groupName).distinct()).hasSize(52);
    assertThat(meta.values().stream().map(Metadata::groupNameRu).distinct()).hasSize(52);
    assertThat(meta.values().stream().map(Metadata::xmlScope).filter(XMLScope.ALL::equals)).hasSize(55);
